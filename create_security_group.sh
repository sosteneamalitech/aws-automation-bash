...
# Define variables 
GROUP_NAME="sostene-devops-lab1-sg"
DESCRIPTION="Automated DevOps Security Group for SSH and HTTP"

echo "Creating security group '${GROUP_NAME}'..."

# Create the security group and capture its ID using the default profile region, tagging it
SECURITY_GROUP_ID=$(aws ec2 create-security-group \
  --group-name "$GROUP_NAME" \
  --description "$DESCRIPTION" \
  --tag-specifications "ResourceType=security-group,Tags=[{Key=Owner,Value=$OWNER},{Key=Project,Value=$PROJECT}]" \
  --query 'GroupId' \
  --output text)
